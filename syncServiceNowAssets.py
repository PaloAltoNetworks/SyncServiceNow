...
def sync_dynamic_address_groups(device, objects, no_delete=False):

    reg_list = []
    unreg_list = []

    if not no_delete:
        logging.info("Getting current tags from firewall for comparison")
        ...
        for reg_address in current_tags:
            ...
            if not tags_in_servicenow:
                logging.debug("Found tag to delete: %s" % (reg_address,))
    ...
    logging.info("Begin sync with firewall dynamic address groups")
...
def main():
    ...
    logging.info("Preparing connection to firewall at %s" % args.fw_hostname)
    fw = device.PanDevice(args.fw_hostname,
                          args.fw_port,
                          args.fw_creds.split(':')[0],
                          args.fw_creds.split(':')[1]
                          )

    sync_dynamic_address_groups(fw, interesting_assets, args.no_delete)
